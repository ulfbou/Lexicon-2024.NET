...
[ApiController]
public class GamessController(IUoW unitOfWork, IMapper mapper, ILogger<TournamentsController> logger) : ControllerBase
{
    ...
    public async Task<ActionResult<GameDto>> PostGame(GameDto gameDto)
    {
        ...
        if (string.IsNullOrEmpty(game.Title) || game.StartDate == default || game.Games == null)
        {
            string[] missingFields = {
                string.IsNullOrEmpty(game.Title) ? "title" : "",
                game.StartDate == default ? "startDate" : "",
                game.Games is null ? "games" : ""
            };
    ...
    public async Task<IActionResult> PutGame(int id, GameDto dto)
    {
        ...
        if (string.IsNullOrEmpty(game.Title) || game.StartDate == default || game.Games == null)
        {
            string[] missingFields = {
                string.IsNullOrEmpty(game.Title) ? "title" : "",
                game.StartDate == default ? "startDate" : "",
                game.Games == null ? "games" : ""
            };

            _logger.LogWarning($"Bad request: Incomplete body for game ID {id}, Missing required fields: {string.Join(", ", missingFields.Where(f => f != null))}");
