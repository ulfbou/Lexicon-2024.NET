...
namespace MachineryFleet.Web
{
    ...
    {
        public static async Task InitializeData(this IServiceCollection collection, IConfiguration configuration)
        {
            //collection.Get
        }
            ...
            using (var scope = app.ApplicationServices.CreateScope())
            {
                    ...
                    })
                {
                    context.Machines.Add(new Machine
                    {
                        ...
                        Status = random.Next(0, 2) == 0 ? MachineStatus.Online : MachineStatus.Offline,
                        LogEntries = []
                    });
                }
        ...
        internal static void InitializeRepository(IApplicationBuilder app)
        {
            using (var scope = app.ApplicationServices.CreateScope())
            {
                var serviceProvider = scope.ServiceProvider;
                ...
                var repository = unitOfWork.GetRepository<Machine, Guid>();
                try
                {
                    repository.AddAsync(new Machine
                    {
                        Id = Guid.NewGuid(),
                        Name = "Machine A",
